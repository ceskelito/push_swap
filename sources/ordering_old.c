...
static void optimize_moves(t_moves_set *moves)
{
	...
	else if(moves->a->dir == 2 || moves->b->dir == 2)
	{
		if(moves->a->dir == 2)
...
void	find_three_biggest(t_stack *a)
{
	int	i;

	a->max[0] = a->max[1] = a->max[2] = INT_MIN;
	i = 0;
	while (i < a->lenght)
	{
		if (a->list[i] > a->max[0])
		...
		else if (a->list[i] > a->max[1])
		{
			a->max[2] = a->max[1];
			a->max[1] = a->list[i];
		}
		else if (a->list[i] > a->max[2])
		{
			a->max[2] = a->list[i];
		}
		i++;
...
int is_in(int *array, int to_find, int size)
{
	int i;
...
/* potrei salvare tutte le mosse in un a->listay di puntatori a funzione e poi eseguirle*/
